    ...
    useEffect(() => {
        const fetchTrackingList = async () => {
            try {
                ...
                const response = await fetch('https://cost-catcher-react.vercel.app/api/trackinglist', {
                    method: 'GET',
                    headers: {
        ...
        <div className='font-inter'>
            {user ? (
                <div className='flex flex-col items-center gap-4'>
                    <h1 className='text-3xl'>Hello <b>{user.name}</b>, welcome to your dashboard!</h1>
                    <div>
                            ...
                            <ul className='space-y-4'>
                                {trackingItems.map((item, index) => (
                                    <li key={index} className='border p-4 rounded-md shadow'>
                                        <div>
                                            <h2 className='text-lg'>Product Title: {item.productTitle}</h2>
                                            <h2 className='text-lg'>Product Link: {item.productLink}</h2>
                                            <h3 className='font-semibold'>Current Price: ₹{item.currentPrice}</h3>
                                            <h4 className='font-semibold'>Target Price: ₹{item.hitPrice}</h4>
                                        </div>
                                        <hr />
                    ...
                    <div className='flex mt-6'>
                        ...
                        <a href='/addProduct' className='bg-black hover:bg-gray-700 text-white font-bold py-2 px-4 rounded'>
                            Track a new Product
                        </a>
                    </div>
                </div>
